...
import { TransactionSteps, TransactionPaymentMethod } from "@/types/transaction";
...
const PriceSummary = ({
  ...
  const subtotal = nights > 0 ? property.room.basePrice * nights * Math.max(bookedUnits, 1) : 0;
        ...
        <div className="space-y-3 text-sm">
          <div className="flex justify-between">
            <span className="text-muted-foreground">
              Rp {property.room.basePrice.toLocaleString("id-ID")} ×{" "}
              {nights > 0 ? `${nights} nights` : "—"}
              {bookedUnits > 1 ? ` × ${bookedUnits} units` : ""}
            </span>
            <span className="font-medium">
              {nights > 0 ? `Rp ${subtotal.toLocaleString("id-ID")}` : "—"}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Service fee (10%)</span>
            <span className="font-medium">
              {nights > 0 ? `Rp ${serviceFee.toLocaleString("id-ID")}` : "—"}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Taxes (5%)</span>
            <span className="font-medium">
              {nights > 0 ? `Rp ${taxes.toLocaleString("id-ID")}` : "—"}
            </span>
          </div>
          <Separator />
